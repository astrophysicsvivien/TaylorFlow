...
def PN_amplitude(freq,ChirpMass,LuminosityDistance):
    ...
    term1 = tf.math.reciprocal(LuminosityDistance/299792458)# divide by factor of c to convert to units of time
...
def getwaveform(mass1,mass2,frequencies=None,LuminosityDistance=1.,f_low=10.,
    ...
    ind2 = tf.where(cond2)
    amp = tf.compat.v1.scatter_update(tf.Variable(amp), ind1, tf.constant(0.))
    amp = tf.compat.v1.scatter_update(tf.Variable(amp), ind2, tf.constant(0.))
